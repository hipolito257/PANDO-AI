...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const title = clean.title || `${companyName} — Internal Review Letter`;
    const subtitle = clean.subtitle || "Investment Committee Memo";

    // Same OOXML template-cloning machinery as the 2-Pager — an admin-uploaded
    // reference .docx (firmSettings.irlTemplateUrl) is cloned exactly (fonts,
    // colors, page structure); only the body content is replaced.
    const settingsRow = await db.query.firmSettings.findFirst({ where: eq(firmSettings.id, FIRM_SETTINGS_ID) }).catch(() => null);
    const templateUrl = settingsRow?.irlTemplateUrl;

    let buffer: Buffer;
    if (templateUrl) {
      const templateRes = await fetch(templateUrl);
      if (!templateRes.ok) throw new Error("Could not download the admin-configured IRL template");
    ...
    const filename = `${safeCompanyName} - IRL - ${new Date().toISOString().slice(0, 10)}.docx`;
